...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate images with text boxes.")
    ...
    def box_type(values):
        try:
            ...
            raise argparse.ArgumentTypeError(f"invalid box format, expected: xmin,ymin,xmax,ymax, got: {values}")
...
if __name__ == "__main__":
    ...
    images = torch.stack([torch.from_numpy(img) for img in images]).permute(0, 3, 1, 2)
    torchvision.utils.save_image(
        images, 
        os.path.join(args.output_dir, "gen_text_box.png"), 
    ...
    #   --revsion "fp16" --prompt "a dog and an apple" \
    #   --num_images_per_prompt 2 --num_inference_steps 50 \
    #   --ground_phrases "a dog" "an apple" \
    #   --ground_boxes 0.1387,0.2051,0.4277,0.7090 0.4980,0.4355,0.8516,0.7266 \
