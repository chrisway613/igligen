...
class UniqueKeyLoader(yaml.SafeLoader):
    """ A YAML loader that throws an error when duplicate keys are found. """
...
def merge_cli_opt(config, key, value):
    ...
    assert type(original_value) == type(value), f"original type:{type(original_value)} != `cli_opt` type:{type(value)}"
...
def merge_cli_opts(config, cli_opts):
    assert len(cli_opts) % 2 == 0, f"Total num of (key, value) should be even, got: {len(cli_opts)}"
    ...
    for key in b:
        if key in a:
            if isinstance(a[key], dict) and isinstance(b[key], dict):
                ...
                if allow_replace:
                    logger.info(
                        f"Replacing key at {'.'.join(path + [str(key)])} to {b[key]}")
                    a[key] = b[key]
                else:
                    raise ValueError(
